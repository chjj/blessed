...
helpers.tagSubstring = function(input, start, end) {
  var text = '';
  var plainText = '';
  ...
  for (i = 0; i < input.length; i++) {
    if (input[i] == '{') { inTag++; }
    if (inTag == 0) { characterPosition++; }
    if (characterPosition > start && characterPosition <= end && inTag == 0) { plainText += input[i]; }
    if (inTag > 0 || (characterPosition > start && characterPosition <= end && inTag == 0)) { text += input[i]; }
    if (input[i] == '}') { inTag--; }
  }
  return text;
}
