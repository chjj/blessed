...
helpers.hasRaw = function(stream, value) {
  return !!stream.setRawMode
    || (stream === process.stdin)
    || !!(stream._handle && stream._handle.setRawMode && stream.readable);
};
...
helpers.setRawMode = function(stream, value) {
  ...
  if (stream === process.stdin) {
    stream._blessedRaw = value;
    return require('tty').setRawMode(value);
  }
  if (stream._handle && stream._handle.setRawMode && stream.readable) {
    stream._blessedRaw = value;
    return stream._handle.setRawMode(value);
  }
