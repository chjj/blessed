...
List.prototype._listEntryFormatter = null;

List.prototype.useFormatter = function(formatterFn) {
    function defaultListItemFormatter (item) {
        if (item.dir === true && item.symlink === false) {
            item.text = '{light-blue-fg}' + item.name + '{/light-blue-fg}/';
        }
        else if (item.dir === false && item.symlink === true) {
          item.text = '{light-cyan-fg}' + item.name + '{/light-cyan-fg}@';
        }
        return item
    }

    if (formatterFn === null) {
        this._listItemFormatter = defaultListItemFormatter;
    }
    else {
        this._listItemFormatter = formatterFn;
    }
}
