...
main:
  for (; no < lines.length; no++) {
    ...
    // If the string is apparently too long, wrap it or crop it.
    while (line.length > width) {
      ...
      for (i = 0, total = 0; i < line.length; i++) {
        ...
        if (++total === width) {
          if (!wrap) {
            // Append all remaining escape codes to close open ones.
            j = i;
            k = i + 1;
            while (match = line.slice(j).match(codeRegExp)) {
              j += match.index + match[0].length;
              append += match[0];
              i += match[0].length;
            }
            line = line.slice(0, k) + append + line.slice(k);
          } else if (line[i] !== ' ') {
