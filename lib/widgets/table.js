...
Table.prototype._calculateMaxes = function() {
  ...
  if (this.options.format) {
    this.options.format.forEach(function (column, i) {
      if (column && column['min-width']) { maxes[i] = Math.max(column['min-width']-1, maxes[i]); }
      if (column && column['max-width']) { maxes[i] = Math.min(column['max-width']-1, maxes[i]); }
    });
  }
